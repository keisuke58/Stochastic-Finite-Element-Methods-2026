...
\author{Keisuke Nishioka \quad {\normalsize Matriculation No.\ 10081049}}
...
\begin{center}
\includegraphics[height=0.70\textheight]{fairing_geometry.png}\\[0.3em]
{\tiny Ogive nose over a cylindrical barrel. The UQ study takes a
representative wall panel of this shell.}
\end{center}
\end{column}
...
\begin{center}
\includegraphics[height=0.70\textheight]{fairing_odb_mises.png}\\[0.3em]
{\tiny One deterministic solve at one quadrature node.}
\end{center}
\end{column}
\begin{column}{0.44\textwidth}
\small
...
\begin{column}{0.54\textwidth}
\centering
\includegraphics[width=\linewidth]{sobol_max_smises.pdf}\\[0.4em]
{\scriptsize $S_i$ (blue) beside $S_T^i$ (red). The pairs coincide, so no
variance is held in interactions; the cohesive bars vanish because damage never
initiates.}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.40\textwidth}
\centering
\includegraphics[width=\linewidth]{t13_rmse_convergence.pdf}\\[0.3em]
{\tiny Test RMSE vs training-set size, median and IQR over 5 seeds. The NN
curve turns \emph{upward} --- more data, worse fit.}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.56\textwidth}
\centering
\includegraphics[width=1.06\linewidth]{reliability_margin.png}\\[0.5em]
{\scriptsize Density and limit on one axis. The PDF is the narrow spike on the
left: $\pm5\sigma$ spans $\approx\!24$~MPa against a 990~MPa margin. No tail
shape survives that separation --- hence $P_f=0$ exactly.}
\end{column}
\begin{column}{0.44\textwidth}
\[
...
\end{itemize}
\begin{alertblock}{Scope of the claim}
\footnotesize Conservative for \textbf{average-governed} responses --- not for peak stress.
\end{alertblock}
\end{column}
\begin{column}{0.46\textwidth}
\includegraphics[width=\linewidth]{kl_effective_variance.pdf}\\[0.4em]
{\tiny Std of $\overline{E_1}$ vs KL truncation $M$, three correlation-length
pairs; dashed $=$ scalar model, 7.3~GPa. Longer correlation $\Rightarrow$ less
averaging $\Rightarrow$ closer to the scalar case.}
\end{column}
\end{columns}
\end{frame}
...
\end{table}
\vspace{-0.3em}
\begin{alertblock}{Read this correctly}
...
\includegraphics[width=\linewidth]{t13_rmse_convergence.pdf}\\[0.3em]
{\scriptsize Test RMSE vs training-set size, median and IQR over 5 seeds.
Only the GP carries an error bar of its own: epistemic std 0.008~MPa against an
aleatory scatter of 2.37~MPa $=$ \textbf{0.3\%}, so surrogate error is
negligible beside the physical variability.}
\end{column}
\end{columns}
\end{frame}
...
\vspace{-0.2em}
\begin{alertblock}{What it does and does not do}
...
\end{column}
\begin{column}{0.45\textwidth}
\includegraphics[width=\linewidth]{bayes_convergence_k.pdf}\\[0.4em]
{\footnotesize Posterior mean of $E_1$ and its 95\% credible band against the
number of measurements $k$. Each $k$ uses the mean of $k$ \emph{independent}
draws, so the band contracts as $1/\sqrt{k}$ and stays centred on the true value
(dotted). Dashed grey is the prior 95\% interval --- the width the data has to
beat.}
\end{column}
\end{columns}
\end{frame}
