...
\begin{column}{0.54\textwidth}
\vspace{0.3em}
Anisotropic exponential kernel, discretised by \textbf{KL expansion}:
\[
  C = e^{-|dx|/l_{cx}-|dy|/l_{cy}},\qquad
  Z(x)=\sum_{i}^{M}\sqrt{\lambda_i}\phi_i(x)\zeta_i
\]
A Gaussian field admits negative stiffness $\Rightarrow$ \textbf{translation field}
$H=F^{-1}(\Phi(Z))$, lognormal marginal, positive by construction.
\vspace{0.3em}

Transform preserves rank but \emph{not} linear correlation:
$\rho_H=\frac{e^{\sigma_{\ln}^2\rho_Z}-1}{e^{\sigma_{\ln}^2}-1}$
--- distortion $3\!\times\!10^{-4}$ at CoV~5\%, $3.8\!\times\!10^{-2}$ at 60\%.
\vspace{0.3em}

Spatial averaging: $\sigma(\overline{E_1})\approx 4{,}700 < 7{,}300$~MPa.
\begin{alertblock}{}
Conservative for \textbf{average-governed} responses --- not proven for \emph{peak} stress.
\end{alertblock}
\end{column}
\begin{column}{0.46\textwidth}
...
\vspace{0.3em}
\textbf{Key result:} posterior std of $E_1$ falls from 7{,}300~MPa to
