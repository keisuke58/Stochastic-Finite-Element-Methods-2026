...
\begin{enumerate}
    ...
    \item Represent $E_1$ as a spatially varying Gaussian random field via KL expansion and quantify the spatial averaging effect on response variability (Appendix~\ref{app:kl}).
...
\textit{Remark (spatial random field):} A spatially varying representation of $E_1$ via Karhunen--Lo\`{e}ve (KL) expansion (Appendix~\ref{app:kl}) shows that the spatially averaged modulus $\overline{E_1}$ has a standard deviation of $\approx 5{,}100$~MPa --- lower than the scalar point value of 7{,}300~MPa. Spatial averaging attenuates the effective structural variability; the scalar model used here is therefore \emph{conservative}. Figure~\ref{fig:kl_effvar_inline} illustrates this result.
...
\begin{figure}[ht]
    ...
    For all correlation lengths the effective variability falls below the scalar baseline (dashed),
    confirming that the scalar UQ is conservative relative to a realistic random field model.}
...
%------------------------------------------------------------------
\section{Non-Gaussian Random Fields via Karhunen--Lo\`{e}ve Expansion}
\label{app:kl}
%------------------------------------------------------------------
...
\begin{figure}[ht]
    ...
    \caption{Six realisations of $E_1(x,y)$ with $M=10$ modes (medium correlation length, 87.3\% variance).}
\end{figure}
